...
const addClosingTag = (node) => {
  // answerString += closingTags
  // closingTags = ''
  let closingTag = NODE_TYPES[node?.type]
  if (closingTag) {
    answerString += `</${closingTag}>`
  }
}
...
const traverse = (node) => {
  ...
  function loopNodes(currentNode) {
    ...
    for (let i = 0; i < currentNodes.length; i++) {
      ...
      if (PRIMITIVES[selectedNode.type]) {
        PRIMITIVES[selectedNode.type](selectedNode)
      }

      if (!currentNodes[i].children) {
        addClosingTag(currentNode[i])
      }
